...
use super::super::model_types::{ConstantDrawableFlagSet, ParameterType};
...
type CLogFn = extern "C" fn(*const core::ffi::c_char);
type BoxedCLogFn = Box<CLogFn>;

struct LogHook {
  trampoline_ptr: *mut Box<CLogFn>,
  user_cb: Box<dyn FnMut(&str) + Send + 'static>,
}

unsafe impl Send for LogHook {}
unsafe impl Sync for LogHook {}

impl Drop for LogHook {
  fn drop(&mut self) {
    unsafe {
      // SAFETY: pointer created via Box::into_raw; convert back to drop correctly.
      let boxed_box = Box::from_raw(self.trampoline_ptr as *mut BoxedCLogFn);
      drop(*boxed_box);
    }
  }
}
...
impl PlatformCubismCoreInterface for PlatformCubismCore {
  ...
  {
    extern "C" fn trampoline(msg: *const core::ffi::c_char) {
      let message_str = unsafe { core::ffi::CStr::from_ptr(msg).to_str().unwrap() };
      if let Some(hook) = LOG_HOOK.lock().unwrap().as_mut() {
        (hook.user_cb)(message_str);
      }
    }

    let boxed_trampoline = BoxedCLogFn::new(trampoline);
    let trampoline_ptr: *mut Box<CLogFn> = Box::into_raw(Box::new(boxed_trampoline)).cast();

    *LOG_HOOK.lock().unwrap() = Some(LogHook {
      trampoline_ptr,
      user_cb: Box::new(f),
    });

    csmSetLogFunction(Some(**trampoline_ptr));
  }
  ...
  fn latest_supported_moc_version(&self) -> MocVersion {
    let csm_moc_version = unsafe { csmGetLatestMocVersion() };
    csm_moc_version
      .try_into()
      .unwrap()
  }
...
impl PlatformMocInterface for PlatformMoc {
  ...
  fn new_platform_model(&self) -> (Self::PlatformModelStatic, Self::PlatformModelDynamic) {
    ...
    let canvas_info = unsafe {
      // `Vector2` 与 `csmVector2` 在对齐和大小上是一致的（见上方 assert），
      // 因此可以安全地通过指针转换，把 C 层写入的值读成 `Vector2`。
      let mut size_in_pixels = Vector2 { x: 0.0, y: 0.0 };
      let mut origin_in_pixels = Vector2 { x: 0.0, y: 0.0 };
      let mut pixels_per_unit: f32 = 0.0;

      csmReadCanvasInfo(
        csm_model,
        &mut size_in_pixels as *mut Vector2 as *mut csmVector2,
        &mut origin_in_pixels as *mut Vector2 as *mut csmVector2,
        &mut pixels_per_unit,
      );
...
impl<'a> VertexPositionContainers<'a> {
  ...
  /// Refresh slices in-place to avoid reallocations on every update.
  /// Assumes drawable count is stable for the lifetime of the model.
  fn refresh(&mut self, csm_model: *mut csmModel) {
    unsafe {
      let drawable_count: usize = csmGetDrawableCount(csm_model).try_into().unwrap();
      debug_assert_eq!(drawable_count, self.inner.len(), "Drawable count changed unexpectedly");

      let vertex_counts = std::slice::from_raw_parts(csmGetDrawableVertexCounts(csm_model), drawable_count);
      let vertex_position_ptrs = std::slice::from_raw_parts(csmGetDrawableVertexPositions(csm_model), drawable_count);

      for (slot, (&vertex_count, &vertex_position_ptr)) in self.inner.iter_mut().zip(itertools::izip!(vertex_counts, vertex_position_ptrs)) {
        let vertex_count: usize = vertex_count.try_into().unwrap();
        *slot = std::slice::from_raw_parts(vertex_position_ptr.cast::<Vector2>(), vertex_count);
      }
    }
  }
}
