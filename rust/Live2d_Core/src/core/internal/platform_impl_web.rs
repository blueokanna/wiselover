...
pub struct PlatformCubismCore {
  js_cubism_core: Rc<JsLive2DCubismCore>,
}
...
impl PlatformCubismCore {
  pub async fn new() -> Self {
    ...
    let js_cubism_core= JsLive2DCubismCore::new(&document).await;
    let js_cubism_core = Rc::new(js_cubism_core);
    Self {
      js_cubism_core,
    }
  }
...
impl PlatformCubismCoreInterface for PlatformCubismCore {
  ...
  fn check_moc_consistency(&self, bytes: &[u8]) -> bool {
    let Ok((_, platform_moc)) = self.platform_moc_from_bytes(bytes) else {
      return false;
    };
    ...
    platform_moc.check_consistency_js_array_buffer(self.js_cubism_core.has_moc_consistency_method(), array.buffer())
  }
  ...
  fn platform_moc_from_bytes(&self, bytes: &[u8]) -> Result<(MocVersion, self::PlatformMoc), MocError> {
    ...
    js_moc
      .map(|js_moc| {
        ...
        PlatformMoc {
          js_moc,
          js_cubism_core: Rc::clone(&self.js_cubism_core),
        })
      })
...
impl PlatformMoc {
  fn check_consistency_js_array_buffer(&self,
    has_moc_consistency_method: &js_sys::Function,
    array_buffer: js_sys::ArrayBuffer,
  ) -> bool {
    self.js_moc.check_consistency_js_array_buffer(has_moc_consistency_method, array_buffer)
  }
}
...
mod js {
  ...
  pub struct JsLive2DCubismCore {
    ...
    /// The `Live2DCubismCore.Moc.hasMocConsistency` method.
    has_moc_consistency_method: js_sys::Function,
  ...
  impl JsLive2DCubismCore {
    pub async fn new(document: &web_sys::Document) -> Self {
      #![allow(non_snake_case)]

      {
        let core_script = document.create_element("script").unwrap();
        let core_script: web_sys::HtmlScriptElement = core_script.dyn_into().unwrap();
        core_script.set_type("text/javascript");
        core_script.set_text_content(Some(LIVE2DCUBISMCORE_JS_STR));

        let head = document.head().unwrap();
        head.append_child(&core_script).unwrap();
      }

      // Allow the WebAssembly module to load.
      {
        while js_sys::eval("Live2DCubismCore.Version.csmGetVersion();").is_err() {
          log::info!("Waiting for Live2DCubismCore to load...");
          fluvio_wasm_timer::Delay::new(std::time::Duration::from_millis(10))
            .await
            .unwrap();
        }
      }

      let code = "Live2DCubismCore";
      ...
      let cubism_version = {
        ...
        let call_result = csmGetVersion.call0(&version_class);
        core::CubismVersion(call_result.unwrap().as_f64().unwrap() as u32)
      };
      ...
      let moc_prototype = get_member_value(&moc_class, "prototype");
      let has_moc_consistency_method = get_member_function(&moc_prototype, "hasMocConsistency");
      ...
      let drawables_prototype = get_member_value(&drawables_class, "prototype");
      let reset_dynamic_flags_method = get_member_function(&drawables_prototype, "resetDynamicFlags");
  ...
  impl JsLive2DCubismCore {
    pub fn has_moc_consistency_method(&self) -> &js_sys::Function {
      &self.has_moc_consistency_method
    }
  ...
  impl JsMoc {
    pub fn check_consistency_js_array_buffer(
      &self,
      has_moc_consistency_method: &js_sys::Function,
      array_buffer: js_sys::ArrayBuffer,
    ) -> bool {
      has_moc_consistency_method.call1(&self.moc_instance, array_buffer.as_ref()).unwrap() == 1
    }
  }
