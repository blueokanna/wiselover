...
fn handle_target_native() {
  let cubism_sdk_dir = if let Some(dir) = get_cubism_sdk_dir_optional() {
    ...
    // 没有 SDK 时生成占位绑定，允许 cargo check 通过
    emit_dummy_bindings();
    return;
  };
...
fn calc_lib_dir(cubism_core_dir: &PathBuf, target_os: &str) -> PathBuf {
  ...
  cubism_core_dir.join("lib").join(target_os).join(platform_lib_dir_name)
}
...
fn get_cubism_sdk_dir_optional() -> Option<PathBuf> {
  ...
  // 1) 优先使用环境变量（推荐方式，跨机器更通用）
  if let Ok(dir) = env::var(env_var_name) {
    return Some(PathBuf::from(dir));
  }

  // 2) 如果没有配置环境变量，则尝试使用项目常用的默认路径
  //    这里根据你的说明，优先尝试 `D:\CubismSdkForNative5`
  #[cfg(windows)]
  {
...
fn get_cubism_sdk_core_dir() -> PathBuf {
  get_cubism_sdk_dir_optional().expect("LIVE2D_CUBISM_SDK_NATIVE_DIR environment variable must be set").join("Core")
}
